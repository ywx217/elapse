...
public:
	...
	virtual void Add(TimeUnit expireTime, Handler handle) = 0;
	// returns false if handle not found, otherwise true
	virtual bool Remove(Handler handle) = 0;
	// removes all expired handles and return them, according to the given time.
	virtual std::vector<Handler> PopExpires(TimeUnit now) = 0;
};
